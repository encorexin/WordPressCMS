...
type EndpointModelConfig = {
  label: string;
  endpoint: string;
  models: ModelOption[];
};

const ENDPOINT_MODELS: Record<string, EndpointModelConfig> = {
  openai: {
    label: "OpenAI",
...
const ALL_MODELS: ModelOption[] = Object.values(ENDPOINT_MODELS).flatMap((config) => config.models);
      ...
      {settingsList.length === 0 ? (
        ...
        <div className="grid gap-4">
          {settingsList.map((settings) => (
            <Card
              key={settings.id}
              className={`border-0 shadow-lg ${settings.is_default ? "ring-2 ring-blue-500" : ""}`}
            >
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`p-2 rounded-lg ${settings.is_default ? "bg-gradient-to-br from-blue-500 to-purple-600" : "bg-muted"}`}
                    >
                    ...
                    <div>
                      ...
                      <CardDescription className="text-xs mt-1">
                        {settings.model} ·{" "}
                        {PRESET_ENDPOINTS.find((e) => e.value === settings.api_endpoint)?.label ||
                          "自定义端点"}
                      </CardDescription>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => openEditDialog(settings)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>配置 AI API 信息，用于文章生成</DialogDescription>
          </DialogHeader>
              ...
              {useCustomEndpoint && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    支持的端点格式：OpenAI 兼容格式{" "}
                    <code className="bg-muted px-1 rounded">/v1/chat/completions</code> 或 Anthropic
                    格式 <code className="bg-muted px-1 rounded">/v1/messages</code>
                  </p>
                </>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setFormData((prev) => ({ ...prev, system_prompt: getDefaultSystemPrompt() }))
                  }
                >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setFormData((prev) => ({ ...prev, image_enabled: checked }))
                    }
                  />
                </div>
              </CardHeader>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setFormData((prev) => ({
                          ...prev,
                          image_provider: v as AISettings["image_provider"],
                        }))
                      }
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({ ...prev, image_api_key: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({ ...prev, image_endpoint: e.target.value }))
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({ ...prev, image_model: e.target.value }))
                      }
                    />
                  </div>
                </CardContent>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setFormData((prev) => ({ ...prev, slug_enabled: checked }))
                    }
                  />
                </div>
              </CardHeader>
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setFormData((prev) => ({ ...prev, slug_model: v === "default" ? "" : v }))
                      }
                    >
                      <SelectTrigger>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>确定要删除此配置吗？此操作无法撤销。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <div className="flex items-start space-x-2">
          ...
          <div className="text-sm text-blue-700 dark:text-blue-300">
            ...
            <p className="text-blue-600 dark:text-blue-400">
              您可以创建多个 AI
              配置，在文章生成时选择使用不同的配置。默认配置会在文章编辑器中自动选中。
            </p>
          </div>
        </div>
      </div>
