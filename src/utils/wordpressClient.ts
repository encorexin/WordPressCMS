...
import { ErrorCodes, createError, type Result } from "./errorHandler";
...
export class WordPressClient {
  ...
  private async request<T>(method: string, endpoint: string, body?: unknown): Promise<Result<T>> {
  ...
  async getPosts(options?: { page?: number; perPage?: number; status?: string }): Promise<Result<WordPressPost[]>> {
  ...
  async createPost(title: string, content: string, options?: PublishOptions): Promise<Result<WordPressPost>> {
  ...
  async uploadMedia(file: File, title?: string): Promise<Result<{ id: number; source_url: string }>> {
  ...
  async getTags(perPage = 100): Promise<Result<Array<{ id: number; name: string; slug: string }>>> {
    return this.request<Array<{ id: number; name: string; slug: string }>>("GET", `/tags?per_page=${perPage}`);
  }
  ...
  async getSiteInfo(): Promise<Result<{ name: string; description: string; url: string; timezone_string: string }>> {
    return this.request<{ name: string; description: string; url: string; timezone_string: string }>("GET", "");
  }
}
...
export async function testWordPressConnection(site: WordPressSite): Promise<{ success: boolean; message: string }> {
